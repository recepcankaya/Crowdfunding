...
contract Fund {
    ...
    modifier activeProposal(uint256 proposalIndex) {
        require(block.timestamp < proposals[proposalIndex].deadline , "This proposal does not accept any vote");
        _;
    }

    modifier preventOverFunding(uint256 proposalIndex) {
        require(proposals[proposalIndex].totalContribution <= proposals[proposalIndex].requestedContribution.mul(11).div(10), 
        "You exceeded contribution limit");
        _;
    }

    modifier isProposalPassed(uint256 proposalIndex) {
        require(proposals[proposalIndex].yes > proposals[proposalIndex].no, "Proposal did not pass. Campaign cannot be started!");
        _;
    }

    modifier onlyVoters(uint256 proposalIndex) {
        Proposal storage proposal = proposals[proposalIndex];
        require(proposal.voters[msg.sender] = true, "You did not vote. You cannot contribute to the campaign");
        _;
    }

    modifier timeRestrictionForContribution(uint256 endTime) {
        require(block.timestamp < endTime, "You cannot contribute because the time has expired");
        _;
    }

    function voteProposal(uint256 proposalIndex, Vote vote) public activeProposal(proposalIndex) {
        Proposal storage proposal = proposals[proposalIndex];
        if(vote == Vote.YES) {
            proposal.yes++;
        } else {
        ...
        require(!proposal.voters[msg.sender], "Since you voted, you cannot vote again");
        proposal.voters[msg.sender] = true;
        proposal.numberOfVoters++;
    }

    function contributeToContract(uint256 proposalIndex) public payable onlyVoters(proposalIndex) preventOverFunding(proposalIndex)
    ...
    function transferContributionToCreator(uint256 proposalIndex) public isProposalPassed(proposalIndex) {
        Proposal storage proposal = proposals[proposalIndex];
        require(proposal.isContributionEnded, "Contribution has not ended yet");
        require(address(this).balance != 0, "Contract balance is zero");
        require(address(this).balance >= proposal.totalContribution, "The contract has insufficient balance");
